...
# ╔═╡ 4ec14c11-7d79-4a8d-b631-a400ff9651b6
begin
	using NLsolve
	using JuMP
	using HiGHS
...
# ╔═╡ f19aa2c1-c500-4db1-9e67-213ac459acc6
using Plots
...
function make_mab(trials, prior_p, prior_q)
	...
	for (s, (p, q)) in pairs(i_to_pq)
		if (p + 1, q) in keys(pq_to_i)
			ss = pq_to_i[(p + 1, q)]
			T[s, ss] = p / (p + q)
		end

		if (p, q + 1) in keys(pq_to_i)
			sf = pq_to_i[(p, q + 1)]
			T[s, sf] = q / (p + q)
		end
...
# ╔═╡ 4f6abf7b-44ac-4750-b5ff-d100032ab4eb
begin
	gis = []
	trials_values = 50:10:100
	for trials in trials_values
		T, R = make_mab(trials, 16, 5)
		gi = solve(
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "7d60c6f0c10c6e6cd569426936f153ae3604439e"
...
git-tree-sha1 = "8b34b612b7fbceb64af9fc4b77af9066b4f2f5b2"
